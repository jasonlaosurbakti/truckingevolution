...
$up=mysql_query("update penjualan set status='$status' where no_penjualan='$no'");
if($up){
				echo "<script language='javascript'>alert('Data berhasil di simpan ! '); 
	                         window.location='simpan_update_mr.php?noNota=$no';</script>";
				}else
...
$sgh=$_POST['sgh'];
$ony=$_POST['ony'];
$validasi1=$_POST['val1'];

$ab=mysql_query("UPDATE penjualan SET tgl_penjualan='$cd',validasi1='$validasi1' where no_penjualan='$no'");
if($ab){
				echo "<script language='javascript'>alert('Data berhasil di simpan ! '); 
	                         window.location='simpan_update_mr.php?noNota=$no';</script>";
				}else
